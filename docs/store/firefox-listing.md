...
5. Right-click the extension icon -> Manage Extension -> Extension preferences (or `about:addons` -> gear icon -> Manage Extension Options). The options page shows a token field and an editable quick-prompts JSON editor with a Reset to defaults button.
