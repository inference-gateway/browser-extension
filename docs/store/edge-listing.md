...
with no source fork. The same `dist/` ZIP is uploaded to both stores.
...
Use the same screenshots as the Chrome Web Store listing, captured at 1280x800 on a GitHub issue or PR page:
...
5. Right-click the extension icon -> Options (or edge://extensions -> Details -> Extension options). The options page shows a token field and an editable quick-prompts JSON editor with a Reset to defaults button.
